...
function AcademyDescription({ title, paragraph, buttonProps, isInverted }) {
  return (
    <div className={cn('academy', { 'academy--inverted': isInverted })}>
      <h3 className={cn('academy__title')}>{title}</h3>
      <p className={cn('academy__text')}>{paragraph}</p>
      <div className={cn({ academy__link: isInverted })}>
